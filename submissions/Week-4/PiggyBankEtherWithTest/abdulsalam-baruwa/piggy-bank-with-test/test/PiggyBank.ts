...
//         const targetAmount = 1e18;
//         const withdrawalDate = await time.latest() + (60 * 60 * 24 * 7); // Set withdrawal date to 1 week from now
//         const deployedPiggy = await Piggy.deploy(targetAmount,withdrawalDate, owner );
//         return {deployedPiggy, owner, address1, withdrawalDate}
//     }
// })
...
describe('PiggyBank', () => {
    ...
    describe('Save', () => {
        ...
        it('Contributed Event is emitted', async () => {
            const { piggyBank, owner, address1 } = await loadFixture(deployPiggyBankFixture);
            const amount = 1e9;
            await expect(piggyBank.save({ value: amount })).to.emit(piggyBank, 'Contributed');
    ...
    describe('Withdraw', () => {
        it('Should withdraw money', async () => {
            ...
            const balance = await hre.ethers.provider.getBalance(piggyBank.target);
            const ownerBalanceAfter = await hre.ethers.provider.getBalance(piggyBank.manager());
