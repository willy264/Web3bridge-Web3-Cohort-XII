...
describe("PiggyBank - ERC20 Deposits", () =&gt; {
  const PiggyBankFixture = async () =&gt; {
    ...
    const piggyBank = await PiggyBank.deploy(targetAmount, BigInt(currentDate + 100), manager.address, http://piggyToken.target, http://piggyNFT.target);
  ...
  describe("Deposit ERC20", () =&gt; {
    it("should allow users to deposit ERC20 tokens", async () =&gt; {
      ...
      await piggyToken.connect(contributor1).approve(http://piggyBank.target, 500);
    ...
    it("should mint NFT after 2 contributions", async () =&gt; {
      ...
      await piggyToken.connect(manager).mint(contributor1.address, 1000);
      await piggyToken.connect(contributor1).approve(http://piggyBank.target, 1000);
      await piggyBank.connect(contributor1).save(500);
      await piggyBank.connect(contributor1).save(500);
  ...
  describe("withdrawal", () =&gt; {
    ...
    it("it should withdraw money", async () =&gt; {
      ...
      await piggyToken.connect(contributor1).approve(http://piggyBank.target, 1000);
      await piggyToken.connect(contributor2).approve(http://piggyBank.target, 500);
      ...
      const contractBalanceAfter = await piggyToken.connect(manager).balanceOf(http://piggyBank.target)
    ...
    it("it should emit a withdrawn event", async () =&gt; {
      ...
      await piggyToken.connect(contributor1).approve(http://piggyBank.target, 1000);
      await piggyBank.connect(contributor1).save(1000)
      const contractBalanceBefore = await piggyToken.connect(manager).balanceOf(http://piggyBank.target)
