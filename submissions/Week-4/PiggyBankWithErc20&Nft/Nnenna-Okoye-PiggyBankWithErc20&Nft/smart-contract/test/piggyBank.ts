...
describe('OurPiggyBank', () => {
    async function deployOurPiggyBankFixture() {
        ...
        const erc20Instance = await erc20.deploy('SUN', 'STK');
        const erc721 = await hre.ethers.getContractFactory('OurERC721');
        const erc721Instance = await erc721.deploy('SUN NFT', 'STK-NFT');
    ...
    describe('Withdraw', () => {
        ...
        it('Should not withdraw money if not manager', async () => {
            ...
            await piggyBank.save(amount);
            await time.increase(60 * 60 + 1);
            await expect(piggyBank.connect(address1).withdrawal()).to.be.revertedWith('UNAUTHORIZED ACCESS');
        });
        it('Should not withdraw money before withdrawal date', async () => {
            const { piggyBank, address1, erc20Instance } = await loadFixture(deployOurPiggyBankFixture);
            const amount = 1e9;
            await erc20Instance.approve(piggyBank.target, amount); await piggyBank.save(amount); await expect(piggyBank.withdrawal()).to.be.revertedWith('NOT YET TIME');
           }); 
          }); 
        });
