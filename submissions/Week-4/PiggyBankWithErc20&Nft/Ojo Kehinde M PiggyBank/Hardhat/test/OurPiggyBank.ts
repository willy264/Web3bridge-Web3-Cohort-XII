...
describe("OurPiggyBank", () => {
  async function deployOurPiggyBankFixture() {
    ...
    const erc20Instance = await erc20.deploy("LEO", "LTK");
    const erc721 = await hre.ethers.getContractFactory("OurERC721");
    const erc721Instance = await erc721.deploy("LEO NFT", "LTK-NFT");
    ...
    const piggyBank = await piggyBankFactory.deploy(
      1e9,
      _time,
      owner.address,
      erc20Instance.target,
      erc721Instance.target
    );
    return {
      piggyBank,
      erc20Instance,
  ...
  describe("Save", () => {
    it("Should save money", async () => {
      const { piggyBank, erc20Instance } = await loadFixture(
        deployOurPiggyBankFixture
      );
      const amount = 1e9;
    ...
    it("should save money if not manager", async () => {
      const { piggyBank, address1, erc20Instance } = await loadFixture(
        deployOurPiggyBankFixture
      );
      const amount = 1e9;
    ...
    it("should mint erc721 token", async () => {
      const { piggyBank, erc721Instance, owner, erc20Instance } =
        await loadFixture(deployOurPiggyBankFixture);
    ...
    it("should not accept money after withdrawal date", async () => {
      ...
      await erc20Instance.approve(piggyBank.target, amount);
      await expect(piggyBank.save(amount)).to.be.revertedWith(
    ...
    it("increase contributors count", async () => {
      const { piggyBank, owner, address1, erc20Instance } = await loadFixture(
        deployOurPiggyBankFixture
      );
      const amount = 1e9;
      await erc20Instance.approve(piggyBank.target, amount);
    ...
    it("increase contributors count once for a contributor regardless multiple savings", async () => {
      const { piggyBank, erc20Instance, owner } = await loadFixture(
        deployOurPiggyBankFixture
      );
      const amount = 1e9;
    ...
    it("contribution mapping is updated", async () => {
      const { piggyBank, owner, erc20Instance } = await loadFixture(
        deployOurPiggyBankFixture
      );
      const amount = 1e9;
      await erc20Instance.approve(piggyBank.target, amount);
